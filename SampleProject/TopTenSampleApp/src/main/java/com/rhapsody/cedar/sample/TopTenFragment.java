...
import com.rhapsody.cedar.AuthorizedRequest;
...
import com.rhapsody.cedar.sample.library.tracklistplayer.RepeatMode;
import com.rhapsody.cedar.sample.library.tracklistplayer.TrackList;
...
public class TopTenFragment extends PlayerFragment  implements TrackListPlayer.SequenceChangeListener, AdapterView.OnItemLongClickListener {
    ...
    private void loadTopTracks() {
		metadata.getTopTracks(TRACK_COUNT, 0, new Callback<List<Track>>() {

			@Override
			public void success(List<Track> tracks, Response response) {
                trackAdapter.updateTracks(tracks);
                trackListPlayer.setTrackList(new TrackList(tracks));
			}
    ...
    private void loadListeningHistory() {
        ...
        new AuthorizedRequest<List<Track>>(Rhapsody.getInstance().getSessionManager()) {
            @Override
            protected void onSessionValid() {
                metadata.getTrackService().getListeningHistory(appInfo.getApiKey(), getAuthorizationBearer(), this);
            }

            @Override
            protected void onError(RhapsodyError rhapsodyError, RetrofitError retrofitError) {
