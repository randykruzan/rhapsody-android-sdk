package com.rhapsody.cedar.sample.player;
...
import com.rhapsody.cedar.sample.library.PlayerFragment;
import com.rhapsody.cedar.sample.library.Utils;
import com.rhapsody.cedar.CompletionCallback;
import com.rhapsody.cedar.sample.library.metadata.Station;
import com.rhapsody.cedar.station.PreviewTracksChangeListener;
import com.rhapsody.cedar.station.StationPlayer;
import com.rhapsody.cedar.station.data.StationTrackLikedState;
import com.rhapsody.cedar.station.data.StationTuningParams;
...
public class StationPlayerFragment extends PlayerFragment implements PreviewTracksChangeListener, AdapterView.OnItemClickListener, AdapterView.OnItemLongClickListener, StationPlayer.CreationCallback {
    ...
    private void setupStationPlayer(Station station) {
        ...
        if(stationPlayerFromApp != null && (station.id.equals(stationPlayerFromApp.getStationId()))) {
            if(app.getSessionManager().isSessionOpen()) {
                ...
                onStationPlayerCreationError(new RhapsodyError(RhapsodyError.Type.AUTHENTICATION));
            }
        } else {
            Rhapsody rhapsody = app.getRhapsody();
            rhapsody.createStationPlayer(station.id, this);
        }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if(id == R.id.menu_item_tune) {
            ...
            loadArtistStation(stationPlayer.getCurrentTrack().artist);
            return true;
    ...
    private void loadArtistStation(Track.Artist artist) {
        setupStationPlayer(createStationWith(artist.id, artist.name));
    }

    private Station createStationWith(String id, String name) {
        //TODO might change this one day
        return new Station(id, name + " Radio", "", "", "", "", new Station.ImageUrls("",""));
    }
    ...
    @Override
    public void onTrackLoadError(RhapsodyError rhapsodyError) {
        if(isLoginError(rhapsodyError)) {
            onLoginError();
        } else {
    ...
    private boolean isLoginError(RhapsodyError rhapsodyError) {
        return rhapsodyError.type == RhapsodyError.Type.AUTHENTICATION;
    }
    ...
    @Override
    public void onStationPlayerCreationError(RhapsodyError error) {
